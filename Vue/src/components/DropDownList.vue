...
let resetSelectionFlag = false;
let autoSelectionFlag = false;
...
function onOpened(e: DxDropDownBoxTypes.OpenedEvent): void {
  ...
  if (listFirstLoadCompleted && !isSearchIncomplete(dropDownBox)) {
...
function onSelectionChanged(e: DxTreeListTypes.SelectionChangedEvent): void {
  if (!resetSelectionFlag) {
    const keys = e.selectedRowKeys as number[];
    value.value = keys.length ? keys[0] : null;
    if (!autoSelectionFlag) {
      focusInput();
    }
  }
  resetSelectionFlag = false;
  autoSelectionFlag = false;
}
</script>
